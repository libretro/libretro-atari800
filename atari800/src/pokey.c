...
#ifdef NEW_CYCLE_EXACT
	/* Cycle-precise timer IRQ state: the in-line cycle an IRQ is due at, the
	   timers that are waiting to be taken, and the phase of the 15 kHz clock
	   (set by software leaving SKCTL init mode). All three are live state
	   between scanlines, so runahead/netplay need them. */
	Retro_SaveINT(&POKEY_irq_at_xpos, 1);
	Retro_SaveUBYTE(&POKEY_irq_pending_mask, 1);
	Retro_SaveINT(&irq_15khz_phase, 1);
#endif
}
...
#ifdef NEW_CYCLE_EXACT
	Retro_ReadINT(&POKEY_irq_at_xpos, 1);
	Retro_ReadUBYTE(&POKEY_irq_pending_mask, 1);
	Retro_ReadINT(&irq_15khz_phase, 1);
#endif
}
#endif /* __LIBRETRO__ */
